...
const Formulario = ({ cliente }) => {
    ...
    try {
      if (cliente) {
        const respuesta = await fetch(
          `http://localhost:3001/people/${cliente.id}`,
          {
            method: "PUT",
  ...
  return (
    <div className="bg-white mt-10 px-5 py-10 rounded-md shadow-lg shadow-gray-600 md:w-3/4 mx-auto">
      {cliente ? (
        <h1 className="text-gray-600 font-black text-xl uppercase">
          {" "}
          Editar : {cliente.fullName}
        </h1>
      ) : (
        <h1 className="text-gray-600 font-black text-xl uppercase">
          {" "}
            ...
            <Form className="mt-10">
              ...
              <div className="mb-4">
                ...
                {touched.age && errors.age ? (
                  <Alerta>{errors.age}</Alerta>
                ) : null}
              </div>
              <div className="mb-4">
                <label className="text-gray-800" htmlFor="occupation">
                  Ocupacion:
                </label>
                <Field
                  id="occupation"
                ...
                {touched.occupation && errors.occupation ? (
                  <Alerta>{errors.occupation}</Alerta>
                ) : null}
              </div>
              <div className="mb-4">
                ...
                {touched.gender && errors.gender ? (
                  <Alerta>{errors.gender}</Alerta>
                ) : null}
              </div>
              <div className="mb-4">
                <label className="text-gray-800" htmlFor="picture">
                  url:
                </label>
                <Field
                  id="picture"
              ...
              <div className="flex">
                ...
                <div>
                  <button
                    onClick={() => cancelar()}
                    className="mt-5 ml-10 flex-initial w-32 bg-red-500  p-3 text-white uppercase font-bold text-lg"
                  >
                    cancelar
                  </button>
                </div>
